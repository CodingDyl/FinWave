...
import { updateBeneficiary, listDestinations, updateDestination } from "../../lib/api";
import CreateBankAccountModal from "../destinations/CreateBankAccountModal";
import type { Beneficiary, Destination } from "../../types/index";

type Props = {
  open: boolean;
  onClose: () => void;
  beneficiary: Beneficiary | null;
  onUpdated?: (beneficiary: Beneficiary) => void;
};
...
export default function UpdateBeneficiaryModal({ open, onClose, beneficiary, onUpdated }: Props) {
  ...
  useEffect(() => {
    if (beneficiary && open) {
      const loadDestinations = async () => {
        ...
        try {
          console.log(`Loading destinations for beneficiary ${beneficiary.id}`);
          const data = await listDestinations(beneficiary.id);
          ...
          toast.error({
            ...
            description: "Could not load bank accounts for this beneficiary. Please try again.",
          });
        } finally {
  ...
  useEffect(() => {
    if (open && beneficiary) {
      ...
      setType(beneficiary.type || "individual");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      console.log(`Updating beneficiary ${beneficiary.id}:`, payload);
      const data = await updateBeneficiary(beneficiary.id, payload);
  ...
  const handleBankAccountCreated = (destination: Destination) => {
    console.log("Bank account created:", destination);
    setDestinations(prev => [destination, ...prev]);
    toast.success({
      title: "Bank account added",
      description: `Bank account has been added to ${beneficiary.name}.`,
    });
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* dialog */}
      <div className="relative z-10 w-full max-w-2xl card p-6 border shadow-lg animate-in slide-in-from-bottom-2 duration-200">
        {/* Header */}
        <div className="mb-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h2 className="text-xl font-semibold text-foreground">Update Beneficiary</h2>
              <p className="text-sm text-muted-foreground">
                Edit beneficiary details and manage bank accounts
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div>
            <h3 className="text-lg font-medium text-foreground mb-4">Beneficiary Details</h3>
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                onClick={() => setShowCreateBankAccount(true)}
            ...
            {loadingDestinations ? (
              ...
              <div className="space-y-3">
                {destinations.map((destination) => (
                  <div key={destination.id} className="p-3 rounded-lg border border-border">
